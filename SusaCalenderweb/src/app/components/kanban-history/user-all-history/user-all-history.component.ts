...
@Component({
  selector: 'app-user-all-history',
  templateUrl: './user-all-history.component.html',
  styleUrls: ['./user-all-history.component.scss']
})
export class UserAllHistoryComponent implements OnInit {
 ...
 // Define global variables to hold the current user's _id and userName



  getKanbanEvents() {
    this.kanbanEvents = [];
    this.getCurrentUser(); // Ensure current user is retrieved before making the request

    if (this.currUser && this.currUser._id) {
      // Call the service method to get kanban events for the current user
      this.tasksService.getKanbanEventsByUser(this.currUser._id).subscribe(res => {
        ...
        console.error('Error fetching kanban events for user:', error);
      });
    } else {
  ...
  getCurrentUser(): void {
    ...
    // Check if currUser exists before extracting data
    if (this.currUser && this.currUser._id && this.currUser.userName) {
      // Extract _id and userName and save to global variables
      this.currentUserId = this.currUser._id;
      this.currentUserName = this.currUser.userName;
  
      // Log for debugging purposes
      console.log('Current User ID:', this.currentUserId);
      console.log('Current User Name:', this.currentUserName);
    } else {
      // Handle case if currUser is not found or missing data
      console.log('User data not found in localStorage or missing required fields');
    }
  }
...
// Define a global object to hold the selected event data in the desired format
...
openEditModal(event: any): void {
  ...
  // Store the selected event and sprintId
  this.selectedEventId = this.selectedEvent._id;
  this.ticketsprintId = this.selectedEvent.sprintId;

  // Store the event data in a structured body format
  this.selectedEventData = {
    eventId: this.selectedEventId,
    sprintId: this.ticketsprintId,
    title: this.selectedEvent.title,
    ticketId: this.selectedEvent.ticketId,
  ...
  console.log('Selected Event Data:', this.selectedEventData);

  // Open the edit modal with the selected event data
  this.dialog.open(this.editModal, {
    data: { ...this.selectedEventData }  // Pass the structured data to the modal
  });
}
updateSprint(): void {
  // Prepare ticket details to be pushed into the tickets array
  const ticketDetails = {
    ticketId: this.selectedEventData.ticketId,
    title: this.selectedEventData.title,
    tickethours: this.selectedEventData.ticketHours,
    _id: this.selectedEventId, // Selected event ID as the ticket's ID
  };

  // Prepare current user details to be pushed into the resources array
  const userDetails = {
    id: this.currentUserId, // Current user's ID
    name: this.currentUserName, // Current user's name
    // Current user's ID as _id (assuming this is needed in resources array)
  };

  // Prepare the sprint data to update
  const sprintData = {
    sprintId: this.ticketsprintId, // Sprint ID from your component
    tickets: [ticketDetails], // Add the ticket details to the tickets array
    resources: [userDetails], // Add the user details to the resources array
  };

  // Log the data for debugging
  console.log('Prepared sprint data:', sprintData);

  // Now update the sprint in the backend (replace with your API call)
  this.sprintService.updateSprints(sprintData).subscribe(
    (response) => {
